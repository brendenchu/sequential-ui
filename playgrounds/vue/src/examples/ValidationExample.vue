  ...
  <div>
          ...
          <div class="max-w-md mx-auto">
            ...
            <div v-if="panel?.id === 'start'" class="text-center space-y-4">
              ...
              <p class="text-gray-700">
                This demo shows different validation scenarios. Try navigating to see how validation
                works!
              </p>
            </div>
            ...
            <div v-else-if="panel?.id === 'basic-validation'" class="space-y-4">
              <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p class="text-blue-700 text-sm">
                  <strong>Validation Rule:</strong> You must enter a name to proceed.
                </p>
              </div>
            ...
            <div v-else-if="panel?.id === 'async-validation'" class="space-y-4">
              <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p class="text-yellow-700 text-sm">
                  <strong>Async Validation:</strong> This simulates checking username availability
                  (2 second delay).
                </p>
              </div>
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  />
                  <div v-if="isValidatingUsername" class="absolute right-3 top-2.5">
                    <div
                ...
                <p v-else-if="userInput.username" class="mt-1 text-sm text-gray-500">
            ...
            <div v-else-if="panel?.id === 'conditional'" class="space-y-4">
              <div class="bg-purple-50 border border-purple-200 rounded-lg p-4">
                <p class="text-purple-700 text-sm">
                  <strong>Conditional Access:</strong> This panel is only accessible if you're a
                  "premium" user.
                </p>
              </div>
              ...
              <div class="text-center">
                ...
                <p class="text-gray-700">Congratulations! You have premium access.</p>
              </div>
            </div>
            ...
            <div v-else-if="panel?.id === 'disabled'" class="space-y-4">
              <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <p class="text-gray-600 text-sm">
                  <strong>Disabled Panel:</strong> This panel is completely disabled and cannot be
                  accessed.
                </p>
              </div>
            ...
            <!-- Success Panel -->
            <div v-else-if="panel?.id === 'success'" class="text-center space-y-4">
              <div class="text-6xl">✅</div>
              <p class="text-gray-700">
                Great! You've successfully navigated through all the validation scenarios.
              </p>
              <button
                @click="resetDemo"
    ...
    <div class="mt-6 p-4 bg-gray-100 rounded-lg">
      ...
      <div class="space-y-3">
        <label class="flex items-center">
          <input type="checkbox" v-model="userSettings.isPremium" class="mr-2" />
        ...
        <label class="flex items-center">
          <input type="checkbox" v-model="userSettings.enableAsyncDelay" class="mr-2" />
          <span class="text-sm text-gray-700">Enable async validation delay</span>
        </label>
      </div>
    </div>
    ...
    <div class="mt-4 p-4 bg-gray-100 rounded-lg">
      ...
      <div class="max-h-32 overflow-y-auto">
        <div v-for="(log, index) in navigationLog" :key="index" class="text-xs text-gray-600 py-1">
          <span class="font-mono">{{ log.timestamp }}</span> - {{ log.message }}
        </div>
      </div>
      <button @click="navigationLog = []" class="mt-2 text-xs text-blue-600 hover:text-blue-800">
        Clear Log
      </button>
    </div>
    ...
    <div class="mt-4 p-4 bg-gray-100 rounded-lg">
      ...
      <div class="text-sm text-gray-600 space-y-1">
        ...
        <div><strong>User Input:</strong> {{ JSON.stringify(userInput) }}</div>
        <div>Settings: {{ JSON.stringify(userSettings) }}</div>
        <div>
          Validation Errors:
          {{ Object.keys(validationErrors).length > 0 ? JSON.stringify(validationErrors) : 'None' }}
        </div>
      </div>
    </div>
  </div>
...
async function handleBeforeNavigate(event: SequentialNavigationEvent): Promise<boolean> {
